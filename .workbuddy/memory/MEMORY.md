...
- 当前 138 项断言全绿（9 个测试类：主循环/工具/上下文/权限/会话/CLI/加固/review-before-write/评测）。

## 已知边界（简历/面试不得越线）
未做：RAG/向量检索、多 Agent 与规划模式、Redis/MySQL（离线评测集已实现 P0 基础版，见 `src/main/java/com/codeagent/eval/`）。
